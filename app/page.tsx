...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    loadAllData()
  }, [fetchInstructores, fetchDisciplinas , fetchClases, fetchPagos])
  ...
  // Actualizado para usar pagoFinal en vez de solo monto
  const pagosStats = {
    total: pagos.length,
  ...
  const instructoresPorOcupacion = instructores.map((instructor) => {
    ...
    // Actualizado para usar pagoFinal
  ...
  const getPeriodoNombre = (): string => {
    if (!periodoActual) return "No seleccionado"
    return `Periodo ${periodoActual?.numero} - ${periodoActual?.año}`
  }
