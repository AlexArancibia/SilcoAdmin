...
import { ThemeRidesQueryParams } from "@/types/schema";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const total = await prisma.themeRide.count({ where });

    // Obtener theme rides con relaciones
    ...
    console.error("Error al obtener theme rides:", error);
    return NextResponse.json(
...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json(themeRide, { status: 201 });
  } catch (error) {
    console.error("Error al crear theme ride:", error);
    return NextResponse.json(
      { error: "Error interno del servidor" },
      { status: 500 }
    );
